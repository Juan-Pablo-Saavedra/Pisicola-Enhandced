...
const validarCorreo = (correo) => /^\w+([.-]?\w+)*@\w+([.-]?\w+)*(\.\w{2,3})+$/.test(correo);
...
nombreUsuario.addEventListener("input", () => {
  validarCampo(nombreUsuario, errorNombreUsuario, "*El nombre debe tener entre 4 y 20 caracteres", nombreUsuario.value.length < 4 || nombreUsuario.value.length > 20);
});

contraseña.addEventListener("input", () => {
  validarCampo(contraseña, errorContraseña, "*La contraseña debe tener entre 8 y 20 caracteres", contraseña.value.length < 8 || contraseña.value.length > 20);
});
...
formulario.addEventListener("submit", (e) => {
  e.preventDefault();
  verificarCamposRequeridos([nombreUsuario, correo, contraseña]);
});
...
correoLogin.addEventListener("input", () => {
  validarCampo(correoLogin, errorCorreoLogin, "*El correo no es válido", !validarCorreo(correoLogin.value));
});

contraseñaLogin.addEventListener("input", () => {
  validarCampo(contraseñaLogin, errorContraseñaLogin, "*La contraseña debe tener entre 8 y 20 caracteres", contraseñaLogin.value.length < 8 || contraseñaLogin.value.length > 20);
});
